...
export function TodaySessionCard({
  ...
  const editableOutcome: UserRecordableCheckInOutcome | null =
    session.status === 'full' ||
    session.status === 'minimum' ||
    session.status === 'manual_skipped' ||
  ...
  const statusLabel =
    session.status === 'manual_skipped'
      ? 'Manual Skipped'
      : session.status === 'automatic_skipped'
        ? 'Automatic Skipped'
        : session.status === 'full'
          ? 'Full'
          : session.status === 'minimum'
      ...
      <CardContent className="grid gap-4 p-5">
        <div className="flex items-start justify-between gap-3">
          <div>
            ...
            <p className="mt-1 text-sm text-[var(--color-text-secondary)]">
              {session.normalTarget.action}
            </p>
            <p className="text-sm text-[var(--color-text-secondary)]">
              Minimum: <span>{session.minimumTarget.action}</span>
            </p>
            <p className="mt-2 text-sm">
              {session.cue.value ??
                (session.scheduledLocalTime
                  ? `Scheduled at ${session.scheduledLocalTime}`
                  : 'No cue')}
            </p>
          </div>
          <button
            type="button"
            aria-label="Help with check-in"
            className="rounded-md p-2 text-[var(--color-text-secondary)] focus-visible:ring-4 focus-visible:ring-[color-mix(in_srgb,var(--color-focus)_24%,transparent)] focus-visible:outline-none"
          >
        ...
        <div className="flex flex-wrap items-center gap-3 text-sm">
          ...
          <span className="inline-flex items-center gap-2 text-[var(--color-text-secondary)]">
            ...
            {syncLabels[session.synchronizationState]}
          </span>
          {habitState === 'paused' ? (
