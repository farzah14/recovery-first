  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent aria-describedby="friction-dialog-description">
        ...
        <DialogDescription id="friction-dialog-description" className="mt-2">
          This optional note stays private and helps you reflect later.
        </DialogDescription>
        <div className="mt-5 grid gap-4">
          <label className="grid gap-2 text-sm font-semibold" htmlFor="friction-reason">
            Reason (optional)
            <select
              id="friction-reason"
              value={frictionCode ?? ''}
              onChange={(event) =>
                setFrictionCode((event.target.value || null) as FrictionReason | null)
              }
              className="min-h-11 rounded-[var(--radius-md)] border border-[var(--color-border-strong)] bg-[var(--color-surface)] px-3 text-sm"
            >
              <option value="">No reason</option>
              {frictionReasons.map((reason) => (
          ...
          <label className="grid gap-2 text-sm font-semibold" htmlFor="friction-note">
            Private note
            <Input
              id="friction-note"
              maxLength={240}
          ...
          <div className="flex flex-wrap justify-end gap-2">
            ...
            <Button
              type="button"
              variant="secondary"
              onClick={() => onSubmit({ frictionCode: null, frictionNote: null })}
            >
              Skip explanation
            </Button>
