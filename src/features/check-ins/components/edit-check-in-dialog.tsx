...
function metricImpact(outcome: UserRecordableCheckInOutcome): string {
  ...
  if (outcome === 'minimum')
    return 'Minimum keeps the habit alive without requiring the full target.';
              ...
              <label className="grid gap-2 text-sm font-semibold" htmlFor="edit-friction-reason">
                ...
                <select
                  ...
                  onChange={(event) => {
                    ...
                    setFrictionCode(
                      frictionReasons.includes(value as FrictionReason)
                        ? (value as FrictionReason)
                        : null,
                    );
                  }}
