    ...
    <section className="grid gap-6">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div className="grid gap-2">
          <Link href="/app/habits" className="text-sm font-semibold text-[var(--color-primary)]">
            Back to Habits
          </Link>
              ...
              <CardContent className="grid gap-4 p-6">
                ...
                <dl className="grid gap-3 text-sm">
                  <div>
                    ...
                    <dd>{detail.currentVersion.normalTarget.action}</dd>
                  </div>
                  <div>
                    <dt className="font-semibold">Minimum</dt>
                    <dd>{detail.currentVersion.minimumTarget.action}</dd>
                  </div>
                  <div>
                    <dt className="font-semibold">Schedule</dt>
                    <dd>{detail.currentVersion.recurrence.kind}</dd>
                  </div>
                  <div>
                    <dt className="font-semibold">Cue</dt>
                    <dd>{detail.currentVersion.cue.value ?? 'None'}</dd>
                  </div>
                </dl>
              </CardContent>
            </Card>
            <Card>
              <CardContent className="grid gap-3 p-6">
                ...
                <p className="text-sm text-[var(--color-text-secondary)]">
                  Version {detail.currentVersion.versionNumber} is the current definition.
                </p>
                <Button asChild variant="secondary">
                  <Link href={`/app/habits/${detail.habit.id}/history`}>View history</Link>
                </Button>
              </CardContent>
            </Card>
          </div>
        ...
        {tab === 'Insights' ? (
          <Card>
            <CardContent className="p-6">
              <h2 className="text-lg font-semibold">Insights unavailable for now</h2>
              <p className="mt-2 text-sm text-[var(--color-text-secondary)]">
                Keep recording sessions to prepare future insights.
              </p>
            </CardContent>
          </Card>
        ) : null}
          ...
          <ol className="grid gap-3">
            {detail.versions.map((version) => (
              ...
              >
                ...
                <p className="text-sm text-[var(--color-text-secondary)]">
                  {version.source} · {version.createdAt.slice(0, 10)}
                </p>
              </li>
            ))}
          </ol>
