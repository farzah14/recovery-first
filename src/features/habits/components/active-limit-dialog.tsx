    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      ...
      >
        ...
        <DialogDescription id="active-limit-description" className="mt-2">
          {isGuest ? <span>Guest allows 3 active habits</span> : 'Your plan has reached its active habit limit'}.
          ...
          {activeHabits.length > 0 ? (
            <ul className="grid gap-2 text-sm text-[var(--color-text-secondary)]">
              {activeHabits.map((habit) => (
                <li key={habit.id} className="rounded-md bg-[var(--color-surface-subtle)] px-3 py-2">
                  {habit.title}
                </li>
              ))}
            </ul>
          ) : (
            <p className="text-sm text-[var(--color-text-secondary)]">No active habit list is available yet.</p>
          )}
          <Button
            type="button"
            variant="secondary"
            onClick={() => onResolve({ action: 'pause_existing', habitId: activeHabits[0]?.id ?? '' })}
          >
            Pause an Active Habit
          </Button>
          {isGuest ? (
            <Button type="button" variant="secondary" onClick={() => onResolve({ action: 'create_account' })}>
              Create Account
            </Button>
          ) : null}
          <Button type="button" variant="secondary" onClick={() => onResolve({ action: 'keep_draft' })}>
            Keep as Draft
          </Button>
          <Button ref={cancelRef} type="button" variant="ghost" onClick={() => onResolve({ action: 'cancel' })}>
            Cancel
          </Button>
        </div>
      </DialogContent>
