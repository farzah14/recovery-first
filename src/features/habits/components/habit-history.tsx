...
function isEditableOutcome(
  status: SessionSummary['status'],
): status is UserRecordableCheckInOutcome {
  return (
    status === 'full' || status === 'minimum' || status === 'manual_skipped' || status === 'excused'
  );
}
    ...
    <div className="grid gap-3">
      {onEdit ? (
        <p className="text-sm text-[var(--color-text-secondary)]">
          Today&apos;s record changes while prior history remains preserved.
        </p>
      ) : null}
      {sessions.length === 0 ? (
        <p className="text-sm text-[var(--color-text-secondary)]">No session history yet.</p>
      ) : null}
            ...
            <div>
              ...
              {session.status === 'automatic_skipped' ? (
                <p className="mt-1 text-xs text-[var(--color-text-secondary)]">
                  Automatically marked skipped after the check-in window closed
                </p>
              ) : null}
            </div>
