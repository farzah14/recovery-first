...
import type { ProductOwner, ProductRepository, RecordCheckInResult } from '@/lib/repositories/product-repository';
...
describe('editCheckIn application service', () => {
  it('validates optional friction and delegates the immutable edit command', async () => {
    const recordCheckIn = vi.fn(async () => ({
    ...
    expect(recordCheckIn).toHaveBeenCalledWith(expect.objectContaining({ frictionCode: null, frictionNote: null }));
  });
});
...
describe('same-day edit entry points', () => {
  it('explains preserved history and exposes the optional skipped friction form', async () => {
    ...
    render(createElement(HabitHistory, {
...
describe('DexieProductRepository.editCheckIn', () => {
  ...
  it('preserves Full history while projecting the latest Minimum outcome', async () => {
    ...
    expect(await database.checkIns.get('check-in-1')).toMatchObject({ replacedById: result.checkInId });
    expect(await database.sessions.get('session-1')).toMatchObject({ status: 'minimum', revision: 3 });
  });

  it('rejects stale revisions and edits after the owner-local same-day cutoff', async () => {
    await expect(repository.editCheckIn({
