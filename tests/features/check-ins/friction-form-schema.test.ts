...
describe('frictionFormSchema', () => {
  it('accepts no friction, controlled reasons, and a private note', () => {
    ...
    expect(frictionFormSchema.parse({ frictionCode: 'no_time', frictionNote: 'Meeting ran late' })).toEqual({
  ...
  it('rejects unknown friction codes and oversized notes', () => {
    expect(() => frictionFormSchema.parse({ frictionCode: 'secret_reason', frictionNote: null })).toThrow();
    expect(() => frictionFormSchema.parse({ frictionCode: 'other', frictionNote: 'x'.repeat(241) })).toThrow();
  });
});
